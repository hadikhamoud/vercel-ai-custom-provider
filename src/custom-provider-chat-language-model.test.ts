...
const model = provider.chat('customprovider-model-small');
...
describe('doGenerate', () => {
  ...
  function prepareJsonResponse({
    ...
    server.responseBodyJson = {
      ...
      created: 1711113008,
      model: 'customprovider-model-small',
      choices: [
        {
          index: 0,
          message: {
  ...
  it('should pass headers', async () => {
    ...
    await provider.chat('customprovider-model-small').doGenerate({
      inputFormat: 'prompt',
      mode: { type: 'regular' },
      prompt: TEST_PROMPT,
      headers: {
...
describe('doStream', () => {
  ...
  function prepareStreamResponse({ content }: { content: string[] }) {
    server.responseChunks = [
      `data:  {"id":"6e2cd91750904b7092f49bdca9083de1","object":"chat.completion.chunk",` +
        `"created":1711097175,"model":"customprovider-model-small","choices":[{"index":0,` +
  ...
  it('should stream tool deltas', async () => {
    server.responseChunks = [
      `data: {"id":"ad6f7ce6543c4d0890280ae184fe4dd8","object":"chat.completion.chunk","created":1711365023,"model":"customprovider-model-small",` +
    ...
    const { stream } = await createCustomProvider({
      apiKey: 'test-api-key',
      generateId: () => 'test-id',
    })
      .chat('customprovider-model-small')
      .doStream({
        inputFormat: 'prompt',
        mode: {
          type: 'regular',
  ...
  it('should pass headers', async () => {
    ...
    await provider.chat('customprovider-model-small').doStream({
      inputFormat: 'prompt',
      mode: { type: 'regular' },
      prompt: TEST_PROMPT,
      headers: {
